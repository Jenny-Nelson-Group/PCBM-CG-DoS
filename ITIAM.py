...
def timeevolution(Evecs,Pvecs):
    psi0=Evecs[:,0]
    psi1=Pvecs[:,0]

    hbar=27.211     #Scaled to 1 atomic second
    dt=1
    z=1j
    psi_t=np.inner(np.exp(-z*H*dt/hbar),psi1)
    ...
    pl.fill_between(range(n),0,psi0_occ, facecolor='r')
    pl.fill_between(range(n),0,psi1_occ,facecolor='y')
    pl.fill_between(range(n),0,psi_t_occ, facecolor='b')
